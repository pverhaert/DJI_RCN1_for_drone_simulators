...
if SHOW_DEBUG == 1:
    ...
    print( "\n\u001b[31;1mDetailed information is OFF for better performance" )
...
try:
    while True:
        ...
        if bad_length == 1 and SHOW_DEBUG == 1:
            ...
            if read_measure_packets_total == 1:
                ...
                if time_delta_measure_packet > 20000000:
                    ...
                    if SHOW_GT20 and SHOW_DEBUG:
                        print("\n\n\u001b[36;1m", datetime.now(), "Time measure packets gt20 (ms)= ",time_delta_measure_packet/1000000 ) 
            ...
            if len(data) == 38:
                ...
                if SHOW_DEBUG == 1 and ( olx != state['lx'] or oly != state['ly'] or orx != state['rx'] or ory != state['ry'] or ocm != state['camera'] ):
                    print( "\n\u001b[33;1mLX =", '{:6d}'.format( state['lx'] ) ,  ", LY =", '{:6d}'.format( state['ly'] ) 
                                , ", RX =", '{:6d}'.format( state['rx'] ), ", RY =", '{:6d}'.format( state['ry'] ) 
                                , ", Camera =", '{:6d}'.format( state['camera'] ), "\u001b[2A", flush = True )
    ...
    print("\u001b[33mtotal_time (s)                        =", '{:>19,.2f}'.format( (ns_time_stop_global - ns_time_start_global)/ (10 ** 9) ).replace(","," ") )
    ...
    print("\u001b[37mread_time_total (s)                   =", '{:>19,.2f}'.format(read_time_total/ (10 ** 9) ).replace(","," ") )
    print("sent_time_total (s)                   =", '{:>19,.2f}'.format(sent_time_total/ (10 ** 9) ).replace(","," ") )
    print("\u001b[33mTotal READ effective (b/s)            =", '{:>19,.2f}'.format(read_bits_total / ( read_time_total / (10 ** 9) ) ).replace(","," ") )
    print("Total SENT effective (b/s)            =", '{:>19,.2f}'.format(sent_bits_total / ( sent_time_total / (10 ** 9) ) ).replace(","," ") ) #probably it is bandwidth to windows buffer, values like 1 Mb/s
    print("\u001b[37mTotal READ bits                       =", '{:>16,.0f}'.format(read_bits_total ).replace(","," ") )
    print("Total SENT bits                       =", '{:>16,.0f}'.format(sent_bits_total ).replace(","," ") )
    print("\u001b[33mTotal READ packets                    =", '{:>16,.0f}'.format(read_packets_total ).replace(","," ") )
    print("Total READ packets/s                  =", '{:>19,.2f}'.format(read_packets_total / ( (ns_time_stop_global - ns_time_start_global)/ (10 ** 9) ) ).replace(","," ") )        
    print("\u001b[37mTotal READ measure packets            =", '{:>16,.0f}'.format(read_measure_packets_total ).replace(","," ") )
    print("Total READ measurure packets/s        =", '{:>19,.2f}'.format(read_measure_packets_total / ( (ns_time_stop_global - ns_time_start_global)/ (10 ** 9) ) ).replace(","," ") )    
    print("\u001b[33mTotal SENT packets                    =", '{:>16,.0f}'.format(sent_packets_total ).replace(","," ") )
    print("Total SENT packets/s                  =", '{:>19,.2f}'.format(sent_packets_total / ( (ns_time_stop_global - ns_time_start_global)/ (10 ** 9) ) ).replace(","," ") )
    print("\u001b[37mAVG measure packet difference (ms)    =", '{:>19,.2f}'.format(time_sum_between_measure_packets/ (10 ** 6) / ( read_measure_packets_total - 1 ) ).replace(","," ") )
    print("MAX measure packet difference (ms)    =", '{:>19,.2f}'.format(max_time_delta_measure_packet/ (10 ** 6) ).replace(","," ") )
    print("\u001b[33mCount of differences '< 2 ms'     (1) =", '{:>16,.0f}'.format(number_of_0002 ).replace(","," "), "   (", '{:>6,.2f}'.format( 100 * number_of_0002/( read_measure_packets_total - 1 ) ), "% )" )
    if number_of_0002 > 0:
        print("AVG difference for   '< 2 ms'    (ms) =", '{:>19,.2f}'.format(time_sum_between_measure_packets_0002 / number_of_0002 / (10 ** 6) ).replace(","," ") )    
    print(     "\u001b[37mCount of differences '2 - 4 ms'   (1) =", '{:>16,.0f}'.format(number_of_0204 ).replace(","," "), "   (", '{:>6,.2f}'.format( 100 * number_of_0204/( read_measure_packets_total - 1 ) ), "% )"  )
    if number_of_0204 > 0:
        print("AVG difference for   '2 - 4 ms'  (ms) =", '{:>19,.2f}'.format(time_sum_between_measure_packets_0204 / number_of_0204 / (10 ** 6) ).replace(","," ") )    
    print(    "\u001b[33mCount of differences '4 - 7 ms'   (1) =", '{:>16,.0f}'.format(number_of_0407 ).replace(","," "), "   (", '{:>6,.2f}'.format( 100 * number_of_0407/( read_measure_packets_total - 1 ) ), "% )"  )
    if number_of_0407 > 0:
        print("AVG difference for   '4 - 7 ms'  (ms) =", '{:>19,.2f}'.format(time_sum_between_measure_packets_0407 / number_of_0407 / (10 ** 6) ).replace(","," ") )    
    print(    "\u001b[37mCount of differences '7 - 20 ms'  (1) =", '{:>16,.0f}'.format(number_of_0720 ).replace(","," "), "   (", '{:>6,.2f}'.format( 100 * number_of_0720/( read_measure_packets_total - 1 ) ), "% )"  )
    if number_of_0720 > 0:
        print("AVG difference for   '7 - 20 ms' (ms) =", '{:>19,.2f}'.format(time_sum_between_measure_packets_0720 / number_of_0720 / (10 ** 6) ).replace(","," ") )    
    print(    "\u001b[33mCount of differences '> 20 ms'    (1) =", '{:>16,.0f}'.format(number_of_2099 ).replace(","," "), "   (", '{:>6,.2f}'.format( 100 * number_of_2099/( read_measure_packets_total - 1 ) ), "% )\u001b[37m"  )
    if number_of_2099 > 0:
        print("AVG difference for   '> 20 ms'   (ms) =", '{:>19,.2f}'.format(time_sum_between_measure_packets_2099 / number_of_2099 / (10 ** 6) ).replace(","," ") )    
